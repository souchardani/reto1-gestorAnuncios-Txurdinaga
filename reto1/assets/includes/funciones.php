<?php
  ...
  // funcion para mostras 3 anuncios
  ...
  // funcion para mostrar mis ultimos 3 anuncios
  ...
  // funcion mostrar anuncios no validados
  ...
  function obtener_mis_comentarios($usuario) {
    global $Conexionbbdd;
    $sql = "SELECT comentario.id, comentario.Anuncio, anuncio.Título, comentario.Texto
            FROM comentario
            JOIN anuncio ON comentario.Anuncio = anuncio.id
            WHERE comentario.Validado = 1 AND comentario.Autor = '$usuario' ";
    $execute = $Conexionbbdd->query($sql);
    return $execute;
  }
